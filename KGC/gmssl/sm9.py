...
def setup (scheme):
    ...
    s = rand_gen.randrange (curve_order)
...
def private_key_extract (scheme, master_public, master_secret, identity):
    ...
    m = h2rf (1, (user_id + '01').encode('utf-8'), curve_order)
    m = master_secret + m
    if (m % curve_order) == 0:
        return FAILURE
    m = master_secret * prime_field_inv (m, curve_order)
...
def public_key_extract (scheme, master_public, identity):
    ...
    user_id = sm3_hash (str2hexbytes (identity))
    h1 = h2rf (1, (user_id + '01').encode('utf-8'), curve_order)
...
def verify (master_public, identity, msg, signature):
    ...
    if (h < 0) | (h >= curve_order):
        return FAILURE
    if is_on_curve (S, b2) == False:
        return FAILURE
